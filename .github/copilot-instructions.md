# Copilot Instructions for gofr-doc

## GENERAL SECTION (ROOT, MACHINE)

APPLIES_TO: all gofr repos.
OVERRIDE: repo-local `.github/copilot-instructions.md` overrides this file on conflict.
...
## PROJECT SECTION (gofr-doc)

PROJECT_PURPOSE: document processing and rendering service.
RUNTIME: Python (UV).
ENV: VS Code dev container `gofr-doc-dev` on Docker network `gofr-net`.
MCP_TOOLS: none registered yet (server skeleton at `app/mcp_server/mcp_server.py`).
...
| `scripts/bootstrap_gofr_doc.sh` | Bootstrap gofr-doc environment. |
