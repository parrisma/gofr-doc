#!/bin/bash
# =============================================================================
# Ensure gofr-doc Vault AppRole credentials + policies are current
# =============================================================================
...
# bootstrap_gofr_doc.sh run — no manual reprovision needed.
#
# Exit codes:
...
CREDS_FILE="$SECRETS_DIR/service_creds/gofr-doc.json"
ADMIN_CREDS_FILE="$SECRETS_DIR/service_creds/gofr-admin-control.json"
FALLBACK_CREDS_FILE="$FALLBACK_SECRETS_DIR/service_creds/gofr-doc.json"
...
info "Provisioning gofr-doc AppRole (full)..."
if ! command -v uv &>/dev/null; then
    err "uv is required but not available on PATH"
    err "Fix: install uv (project standard) and retry"
    exit 1
