...
Usage:
  ./scripts/bootstrap_gofr_doc.sh [--yes] [--trace] [--log-file PATH] [--no-log]
                               [--start-dev] [--start-prod] [--run-tests] [--auto-tests]
...
This script will:
  ...
  - Build gofr-doc dev and prod images (if missing)
  - Provision Vault AppRole creds for gofr-doc + gofr-admin-control (if needed)
...
setup_logging() {
  ...
  if [[ "$NO_LOG" == "true" ]]; then
    ...
    if [[ -z "$LOG_FILE" ]]; then
      LOG_FILE="${PROJECT_ROOT}/logs/bootstrap_gofr_doc_$(date +%Y%m%d_%H%M%S).log"
    fi
...
build_dev_image() {
  if docker image inspect gofr-doc-dev:latest >/dev/null 2>&1; then
    ok "Dev image exists: gofr-doc-dev:latest"
    return 0
  fi
  ...
  info "Dev image gofr-doc-dev:latest is missing. Building now..."
  (cd "${PROJECT_ROOT}" && bash ./docker/build-dev.sh)
  ok "Dev image built: gofr-doc-dev:latest"
}

build_prod_image() {
  if docker image inspect gofr-doc-prod:latest >/dev/null 2>&1; then
    ok "Prod image exists: gofr-doc-prod:latest"
    return 0
  fi
  ...
  info "Prod image gofr-doc-prod:latest is missing. Building now..."
  (cd "${PROJECT_ROOT}" && bash ./docker/build-prod.sh)
  ok "Prod image built: gofr-doc-prod:latest"
}

ensure_approle_creds() {
  local project_creds="${PROJECT_ROOT}/secrets/service_creds/gofr-doc.json"
  local project_admin_creds="${PROJECT_ROOT}/secrets/service_creds/gofr-admin-control.json"
  local common_creds="${PROJECT_ROOT}/lib/gofr-common/secrets/service_creds/gofr-doc.json"
  ...
  if [[ -f "${project_creds}" && -f "${project_admin_creds}" ]]; then
    ok "AppRole creds already exist for gofr-doc + gofr-admin-control."
    return 0
  fi

  if [[ -f "${common_creds}" && -f "${common_admin_creds}" ]]; then
    ok "AppRole creds already exist for gofr-doc + gofr-admin-control (gofr-common fallback)."
    return 0
  fi
  ...
  info "AppRole creds missing or incomplete (gofr-doc and/or gofr-admin-control). Provisioning now..."
...
start_dev_container() {
  if docker ps --format '{{.Names}}' | grep -q '^gofr-doc-dev$'; then
    ok "Dev container already running: gofr-doc-dev"
    return 0
  fi
  ...
  info "Starting dev container (gofr-doc-dev)..."
...
run_tests_host() {
  ...
  info "Running tests (Docker Compose ephemeral stack)..."
