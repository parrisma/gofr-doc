...
def resolve_templates_dir(cli_dir: Optional[str]) -> str:
    """Resolve templates directory from CLI or project default."""
    if cli_dir:
        return cli_dir
...
def resolve_fragments_dir(cli_dir: Optional[str]) -> str:
    """Resolve fragments directory from CLI or project default."""
    if cli_dir:
        return cli_dir
    
    project_root = Path(__file__).parent.parent
    return str(project_root / "fragments")


def resolve_styles_dir(cli_dir: Optional[str]) -> str:
    """Resolve styles directory from CLI or project default."""
    if cli_dir:
        return cli_dir
    
    project_root = Path(__file__).parent.parent
    return str(project_root / "styles")
        ...
        if args.verbose:
            if args.group:
                ...
                for tmpl in templates:
                    logger.info(f"{tmpl.template_id:<25} {tmpl.name:<30} {tmpl.group:<15} {tmpl.description}")
            else:
                logger.info(f"{'Template ID':<25} {'Group':<15} {'Name':<30} {'Description'}")
                logger.info("-" * 115)
                for tmpl in templates:
                    logger.info(f"{tmpl.template_id:<25} {tmpl.group:<15} {tmpl.name:<30} {tmpl.description}")
        else:
            for tmpl in templates:
    ...
    try:
        ...
        if args.verbose:
            ...
            for frag in fragments:
                ...
                logger.info(f"{frag.fragment_id:<25} {frag.name:<30} {frag.description:<40} {params_count}")
        else:
            for frag in fragments:
...
def list_standalone_fragments(args):
    ...
    fragments_dir = resolve_fragments_dir(args.fragments_dir)
    ...
    try:
        ...
        if args.verbose:
            if args.group:
                logger.info(f"{'Fragment ID':<25} {'Name':<30} {'Group':<15} {'Description':<40} {'Params'}")
                logger.info("-" * 130)
                for frag in fragments:
                    logger.info(f"{frag['fragment_id']:<25} {frag['name']:<30} {frag['group']:<15} {frag['description']:<40} {frag['parameter_count']}")
            else:
                logger.info(f"{'Fragment ID':<25} {'Group':<15} {'Name':<30} {'Description':<40} {'Params'}")
                logger.info("-" * 130)
                for frag in fragments:
                    logger.info(f"{frag['fragment_id']:<25} {frag['group']:<15} {frag['name']:<30} {frag['description']:<40} {frag['parameter_count']}")
        else:
            for frag in fragments:
...
def list_groups(args):
    ...
    templates_dir = resolve_templates_dir(args.templates_dir)
    fragments_dir = resolve_fragments_dir(args.fragments_dir)
    styles_dir = resolve_styles_dir(args.styles_dir)
...
Examples:
  # List all groups
  python render_manager.py groups
  python render_manager.py groups -v

  # List all templates
  python render_manager.py templates list
  python render_manager.py templates list --group public

  # Get template details
  python render_manager.py templates info basic_report

  # List fragments in a template
  python render_manager.py templates fragments basic_report

  # Get fragment details from template
  python render_manager.py templates fragments basic_report --fragment paragraph

  # List standalone fragments
  python render_manager.py fragments list
  python render_manager.py fragments list --group public

  # Get standalone fragment details
  python render_manager.py fragments info news_item

  # Validate template global parameters
  python render_manager.py validate-template basic_report title="My Report" author="John Doe"

  # Validate fragment parameters in a template
  python render_manager.py validate-fragment basic_report paragraph text="Hello world"

  # Validate standalone fragment parameters
  python render_manager.py validate-fragment news_item --standalone headline="Breaking News" body="Story content"

  # Use custom template/fragment/styles directories
  python render_manager.py --templates-dir /custom/templates templates list
  python render_manager.py --fragments-dir /custom/fragments fragments list
  python render_manager.py --styles-dir /custom/styles --group public fragments list

Environment Variables:
    DOCO_DATA_DIR       Override project data directory
        """,
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Templates directory (default: {project}/templates)",
    )
    parser.add_argument(
        "--fragments-dir",
        type=str,
    ...
    templates_subparsers = templates_parser.add_subparsers(dest="templates_cmd", help="Template command")
    ...
    templates_frags.add_argument(
        ...
        type=str,
        default=None,
        help="Get details for a specific fragment"
    )
    templates_frags.add_argument(
        "-v", "--verbose",
        action="store_true",
    ...
    fragments_subparsers = fragments_parser.add_subparsers(dest="fragments_cmd", help="Fragment command")
    ...
    validate_frag.add_argument(
        ...
        type=str,
        default=None,
        help="Template ID (if fragment is in a template)"
    )
    validate_frag.add_argument(
        "--standalone",
        action="store_true",
