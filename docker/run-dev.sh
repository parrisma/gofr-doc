...
if docker ps -q -f name=doco_dev | grep -q .; then
    ...
    echo ""
    echo "To connect from shell: docker exec -it doco_dev /bin/bash"
    echo "To connect from VS Code: use container name 'doco_dev'"
    ...
    echo "Persistent data stored in Docker volume: doco_data_dev"
else
